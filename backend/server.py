...
async def create_url(request: Request):
    try:
        with metrics_handler.query_time.labels("create").time():
        ...
        metrics_handler.http_code.labels("BAD REQUEST").inc()
        raise HTTPException(
            status_code=HttpStatus.BAD_REQUEST.code, detail=str(e))
    except HTTPException as e:
        logging.exception(f"HTTPException: {str(e.detail)}")
        metrics_handler.http_code.labels("INVALID").inc()
        raise e
    except Exception as e:
        logging.exception(f"Internal server error: {str(e)}")
        metrics_handler.http_code.labels(
...
def list_all():
    try:
        with metrics_handler.query_time.labels("list").time():
...
def find_alias(alias: str):
    try:
        with metrics_handler.query_time.labels("find").time():
        ...
        metrics_handler.http_code.labels("NOT_FOUND").inc()
        raise HTTPException(
...
def delete_alias(alias: str):
    try:
        with metrics_handler.query_time.labels("delete").time():
